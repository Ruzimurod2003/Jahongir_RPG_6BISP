...
public class SequenceControlledLight : MonoBehaviour {
    ...
    IEnumerator CoroDoFlash() {
        if (m_lightToFlash == null)
            yield break;
        int numFlashes = UnityEngine.Random.Range(m_minFlashes, m_maxFlashes);
        for (int f = 0; f < numFlashes; ++f) {
            m_lightToFlash.enabled = true;
            yield return new WaitForSeconds(UnityEngine.Random.Range(m_minFlashDuration, m_maxFlashDuration));
            m_lightToFlash.enabled = false;
            yield return new WaitForSeconds(UnityEngine.Random.Range(m_minFlashPauseDuration, m_maxFlashPauseDuration));
        }
    }
}
